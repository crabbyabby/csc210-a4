...
public class Tokenizer {
  ...
  public static Queue<Object> readTokens(String input) {
    ...
    while (scanner.hasNext()) {
      if (scanner.hasNextDouble()) {
        ...
        queue.enqueue(scanner.next(WORD));
      } else if (scanner.hasNext(SYMBOL)) {
        queue.enqueue(scanner.next(SYMBOL).charAt(0));
      } else {
  ...
  /** Run short test
   * @param args should contain a single mathematical expression passed as a String */
